...
st.set_page_config(page_title="Real-time Face Detection", page_icon="./images/faceman_cropped.png", layout="centered")
...
supported_modes = "<html> " \
                  "<body><div> <b>Supported Face Recognize Modes (Change modes from sidebar menu)</b>" \
                  "<ul><li>Image Upload</li><li>Video Upload</li><li>Webcam Video Realtime</li></ul>" \
                  "</div></body></html>"
...
with st.sidebar:
    st.image("./images/faceman_cropped.png", width=260)
    ...
    st.markdown("## Made by **ABC** \U0001F609")
...
if detection_mode == "Image Upload":
    ...
    with col1:
        st.image(image="./images/example_2.png")
    with col2:
        st.image(image="./images/example_3.png")

    uploaded_file = st.file_uploader("Upload Image (Only PNG & JPG images allowed)", type=['png', 'jpg','jpeg'])
...
if detection_mode == "Video Upload":
    ...
    st.info("NOTE: In order to use this mode, you need to upload a video file.")
    ...
    if video_file is not None:
        ...
        with st.spinner("Detecting faces ..."):
            # Read the video file as a bytes object

            vid = video_file.name
            with open(f'storage/video/{vid}', mode='wb') as f:
                f.write(video_file.read()) # save video to disk
            
        video_file_path = f'storage/video/{vid}'
        if video_file_path:
            # Read the video file
